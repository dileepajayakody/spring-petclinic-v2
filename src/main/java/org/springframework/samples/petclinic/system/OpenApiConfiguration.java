...
public class OpenApiConfiguration {
	...
	OpenAPI petClinicOpenApi() {
		return new OpenAPI().info(new Info().title("Spring PetClinic API")
			.description("OpenAPI documentation for the JSON endpoints exposed by Spring PetClinic.")
			.version("v1")
	...
	@Bean
	GroupedOpenApi petClinicApi() {
		return GroupedOpenApi.builder().group("petclinic").displayName("PetClinic API").pathsToMatch("/vets").build();
	}
